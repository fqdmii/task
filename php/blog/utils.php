...
function get_offset(){//offsetよびだす
	$offset = 0;
...
function is_exist($id){//存在しているのか、いるのかどうか
	return get_post($id);
}
...
function update_post($id,$title,$contents,$image){
	$path = "uploads/${id}";
	@mkdir($path,0777,true);#0777よみかき,@は警告をおさえる
	...
	$sql = "update posts set title = ?, contents = ?,
			image_name = ?, image_path = ?, file_type = ?,
			updated = current_timestamp where id = ?";
	$params = array($title, $contents, $image_name, $image_path, $file_type, $id);//順厳しい
